...
insert ignore into ticket_history select ticketid,state,modified,topic,(cast(crc32(ticketid) as decimal(30,30))+crc32(state)+crc32(topic))/(exp(crc32(clientid+state)%512/10)) from client_activity ;




insert into ticket_history 
select ticketid,case when state = 1 then 0 when state = 2 and new_state < 3 then 10 when state = 2 and new_state > 2 then 11 when state = 3 then 1 when state =  4 and new_state < 3 then 2 when state =  4 and new_state > 2 then 3 when state = 10 then 1 when state = 11 then 3 end   , new_modified , case when state = 1 then null else topic end , new_hf from (
	select ticketid,1+(crc32(state)+crc32(topic))%6 as  new_state ,state,from_unixtime(unix_timestamp(modified)-7*(crc32(state+ticketid)%1200)) as new_modified ,topic,(cast(crc32(ticketid) as decimal(30,30))+crc32(state)+crc32(topic))/(exp(crc32(modified)%512/10)) as new_hf from ticket_history 
) f ;

select count(*) from ( select count(*) ,ticketid from client_activity group by ticketid  ) e;

select count(*) from ( select count(*) ,ticketid from ticket_history group by ticketid  ) e;

select count(*),t.state from client_activity a join ticket_history t on a.ticketid=t.ticketid  where a.state=0 group by state ;



select count(*) from client_activity where state <> 0 ;


select count(*),st_path from (select ticketid,GROUP_CONCAT(state order by modified ) as st_path  from ticket_history group by ticketid )e group by st_path ;

select ticketid,GROUP_CONCAT(state)  from ticket_history group by ticketid limit 20 ;



select * from ( select count(*),st_path from (select ticketid,GROUP_CONCAT(state order by modified ) as st_path  from ticket_history group by ticketid )e group by st_path ) g where right(st_path,2) =',2' and right(st_path,5) not in(  ',10,2', ',11,2');


create table test.bad_seq as select * from ( select ticketid,GROUP_CONCAT(state order by modified ) as st_path  from ticket_history group by ticketid )e  where right(st_path,2) =',2' and right(st_path,5) not in(  ',10,2', ',11,2');

update ticket_history set modified=from_unixtime(unix_timestamp(modified)+1) where state in ( 10,11) and ticketid in ( select ticketid from test.bad_seq ) ;

select * from ( select ticketid,GROUP_CONCAT(state order by modified ) as st_path  from ticket_history group by ticketid )e  where right(st_path,2) =',2' and right(st_path,5) not in(  ',10,2', ',11,2');

drop table test.bad_seq ;
create table test.bad_seq as select * from ( select ticketid,GROUP_CONCAT(state order by modified ) as st_path  from ticket_history group by ticketid )e  where st_path not in ('0','0,1','0,1,10,2','0,1,3','0,1,3,11,2','0,1,3,11,2,4','0,1,3,4') ;

update ticket_history set modified=from_unixtime(unix_timestamp(modified)-1) where state in (0) and ticketid in ( select ticketid from test.bad_seq where left(st_path,3) = '1,0' ) ;
update ticket_history set modified=from_unixtime(unix_timestamp(modified)-1) where state in (0,1) and ticketid in ( select ticketid from test.bad_seq where left(st_path,5) = '0,3,1' ) ;

update ticket_history set modified=from_unixtime(unix_timestamp(modified)-1) where state in (0,1,3) and ticketid in ( select ticketid from test.bad_seq where left(st_path,7) = '0,1,4,3' ) ;

update ticket_history set modified=from_unixtime(unix_timestamp(modified)-1) where state in (0,1,3) and ticketid in ( select ticketid from test.bad_seq where left(st_path,12) = '0,1,11,3,2,4' ) ;
